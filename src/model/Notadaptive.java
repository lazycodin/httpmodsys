...
import shedulers.*;
//udp not adaptive system
public class Notadaptive {
    ...
    public Notadaptive(int M, String Sched, int nRB, File fading,
            int vSpeed, int vMes[],
            int vPause[]) {
        ...
        while (currSlot != simTime) {
            ...
            for (int i = 0; i < abonents.length; i++) {
               // abonents[i].Rm = (i + 1) * vSpeed / (abonents.length + 1);
                abonents[i].Rm = R.nextInt(5*1024*1024);
            }

            while (currRB < nRB) {
                //генерирование требования каждым абонентом
                for (int i = 0; i < abonents.length; i++) {
                  tmpQueue.add(new Message(i,abonents[i].size,abonents[i].getMesNotAdaptive(currSlot),0));
                    if (Math.ceil(tmpQueue.element().arrivalTime) <= currSlot) {
                        if (checkUser(tmpQueue.element().numAbonent, activeUsers) == 0) {
                            activeUsers.add(tmpQueue.element().numAbonent); 
                        }
                    }
                }
                
                
                while (tmpQueue.size() > length){
                    tmpQueue.remove();
                    cc++;
                }
...
////                        break;
////                    }
                Abonent  A = new Abonent(tmpQueue.element().numAbonent,slots,vMes,vPause);
                Scheduler.planning(currSlot, activeUsers, A, currRB);
